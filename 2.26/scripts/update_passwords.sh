...
if [[ "${USE_DEFAULT_CREDENTIALS}" =~ [Ff][Aa][Ll][Ss][Ee] ]]; then
  ...
  if [[ -f "${EXTRA_CONFIG_DIR}"/.default_admin_encrypted_pass.txt ]];then
      ...
      #if users.xml has mulitple users, multiple password will be returned
      #export GEOSERVER_ADMIN_DEFAULT_PASSWORD=$(grep -o 'password=\".*\"' ${USERS_XML}|awk -F':' '{print $2}')
      export GEOSERVER_ADMIN_DEFAULT_PASSWORD=$(cat ${USERS_XML} | grep "name=\"${GEOSERVER_ADMIN_DEFAULT_USER}\"" | grep -o "password=\".*\"" |awk -F':' '{print $2}')
  ...
  if [[ ! -f "${SETUP_LOCKFILE}"  ]]; then
      ...
      if [[ -f "${EXTRA_CONFIG_DIR}"/.default_admin_pass.txt ]];then
          ...
          #if hidden file doesn't exist, set to empty password and force to change the admin password in users.xml
          export GEOSERVER_ADMIN_PASSWORD_TEMP=""
      fi
      ...
      if [[ ${GEOSERVER_ADMIN_PASSWORD_TEMP} != ${GEOSERVER_ADMIN_PASSWORD} ]];then
          ...
          PWD_HASH=${GEOSERVER_ADMIN_DEFAULT_ENCRYPTED_PASSWORD}
      fi
  fi
